...
// Reference (desktop) dimensions the design was built around.
const BASE_W = 500
const BASE_H = 220
const EXPANDED_W = 560
const EXPANDED_H = 340
// Horizontal breathing room to leave on very narrow screens (px), so the
...
export function LocationMap({
  ...
  // Track how much horizontal space we actually have, and derive the
  // collapsed/expanded pixel sizes from that instead of using the fixed
  // desktop numbers on every screen. Aspect ratio is preserved so the map
  // artwork inside doesn't get squashed.
  const [availableWidth, setAvailableWidth] = useState<number | null>(null)

  useEffect(() => {
    const measure = () => {
      const parentWidth = wrapperRef.current?.parentElement?.clientWidth ?? window.innerWidth
      setAvailableWidth(Math.max(220, parentWidth - EDGE_MARGIN))
    }
    measure()
  ...
  const collapsedWidth = availableWidth !== null ? Math.min(BASE_W, availableWidth) : BASE_W
  const collapsedHeight = collapsedWidth * (BASE_H / BASE_W)
  const expandedWidth = availableWidth !== null ? Math.min(EXPANDED_W, availableWidth) : EXPANDED_W
  const expandedHeight = expandedWidth * (EXPANDED_H / EXPANDED_W)
  ...
  return (
    <div ref={wrapperRef} className="w-full flex justify-center">
      ...
      >
        <motion.div
          className="relative overflow-hidden rounded-2xl bg-background border border-border max-w-full"
          style={{
        ...
        >
          ...
          <div className="relative z-10 h-full flex flex-col justify-between p-4 sm:p-5">
            ...
            <div className="space-y-1 pr-2">
              <motion.h3
                className="text-foreground font-medium text-sm tracking-tight truncate"
                animate={{
                  x: isHovered ? 4 : 0,
                }}
              ...
              <button
                ...
                className="absolute bottom-3 right-3 sm:bottom-4 sm:right-4 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg bg-amber-500 text-black font-semibold z-50 text-xs sm:text-sm whitespace-nowrap"
              >
                Open Maps
              </button>
